...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	for (int i=0;i<100;i++)
	{
		strcpy(text[i],"Hello world");
		nCount[i]=strlen("Hello world");
	}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		break;
	case WM_LBUTTONDOWN:               ////////////按住鼠标左键不放，生成边框
	...
	case WM_PAINT:
		...
		{
			MoveToEx(hdc,rectDIY[i].left,rectDIY[i].top,0);  //////////////画边框
			LineTo(hdc,rectDIY[i].right,rectDIY[i].top);
			MoveToEx(hdc,rectDIY[i].left,rectDIY[i].top,0);
			LineTo(hdc,rectDIY[i].left,rectDIY[i].bottom);
			MoveToEx(hdc,rectDIY[i].right,rectDIY[i].bottom,0);
			LineTo(hdc,rectDIY[i].right,rectDIY[i].top);
			MoveToEx(hdc,rectDIY[i].right,rectDIY[i].bottom,0);
			LineTo(hdc,rectDIY[i].left,rectDIY[i].bottom);

			DrawText(hdc,text[i],nCount[i],&rectDIY[i],0);
		}
