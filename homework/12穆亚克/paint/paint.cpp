...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		...
		for(int i=0;i<nCount;i++)
		{
			...
			LineTo(hdc, xPosition[i-1], yPosition[i]);
			MoveToEx(hdc, xPosition[i-1], yPosition[i], 0);
			LineTo(hdc, xPosition[i], yPosition[i]);
			MoveToEx(hdc, xPosition[i], yPosition[i], 0);
			LineTo(hdc, xPosition[i], yPosition[i-1]);
			MoveToEx(hdc, xPosition[i], yPosition[i-1], 0);
			LineTo(hdc, xPosition[i-1], yPosition[i-1]);
			}
